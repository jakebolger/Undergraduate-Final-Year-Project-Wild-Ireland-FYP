...
public class GrassGeneratorScript : MonoBehaviour
{
    ...
    //
    public int grassSize = 25;
    //Spacing between grassElements e.g. trees, rocks, bushes etc.
    //
    public int grassSpacing = 3;

    //Array for grassElements to add prefabs to
    //
    public Element[] grassElements;
    ...
    public void Generate(float[,] heightMap)
    {
        ...
        //this means every 3 units in unity we will put i grassElements down
        //
        for (int x = 0; x < grassSize; x += grassSpacing)
        {
            for (int z = 0; z < grassSize; z += grassSpacing)
            {
                for (int i = 0; i < grassElements.Length; i++)
                {
                    //Gets element, giving priority to trees
                    //
                    Element element = grassElements[i];
                    ...
                    if (element.CanPlace())
                    {
                        ...
                        //setting the grassElements position
                        //
...
//
[System.Serializable]
public class Element2 //Element class 
{
